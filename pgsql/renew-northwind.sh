...
check_psql() {
    if command -v psql &> /dev/null; then
        ...
        USE_DOCKER=false
        return 0
    elif docker ps --format '{{.Names}}' | grep -q "pgsql-infra"; then
        echo "Using psql from Docker container"
        USE_DOCKER=true
        return 0
    else
        echo "Error: psql not found and Docker container 'pgsql-infra' is not running"
        echo ""
        echo "Options:"
...
run_psql() {
    if [ "$USE_DOCKER" = true ]; then
        docker exec -i pgsql-infra psql -U postgres "$@"
    else
...
if [ "$USE_DOCKER" = true ]; then
    ...
    docker cp "$SQL_FILE" pgsql-infra:/tmp/northwind.sql
    docker exec -i pgsql-infra bash -c "
...
if [ $? -ne 0 ]; then
    ...
    if [ "$USE_DOCKER" = true ]; then
        echo "   docker exec -i pgsql-infra psql -U postgres -d ${DATABASE} < $SQL_FILE"
    else
...
if [ "$USE_DOCKER" = true ]; then
    echo "Connect using:"
    echo "  docker exec -it pgsql-infra psql -U postgres -d ${DATABASE}"
else
    echo "Connect using:"
